...
private:
    ...
    void seek(size_t wordStart); // locate the activeWord that starts at wordStart
};

// overload the find first set and count leading zeros gcc builtin functions
...
inline int cls(unsigned long long bits) { return __builtin_clsll(bits); }
inline int cls(unsigned long bits)      { return __builtin_clsl (bits); }
inline int cls(unsigned int bits)       { return __builtin_cls  (bits); }
...
void BitVector<T>::seek(size_t wordStart) {
    ...
    while (activeWordStart < wordStart) { // seek forward
        activeWordIdx++;
...
void BitVector<T>::fillSetBits(size_t wordStart, uint32_t *A, uint32_t v) {
    seek(wordStart);
    if (activeWordType == LITERAL) {
        fprintf(stderr,"fillSetBits(%zi,A,%u) LITERAL\n",wordStart,v);
        ...
        fprintf(stderr,"fillSetBits(%zi,A,%u) ONEFILL\n",wordStart,v);
