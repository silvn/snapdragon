...
inline uint64_t revcomp(uint64_t val) {
	...
	return (table[val&0xFFULL]<<56) | (table[(val>>8)&0xFFULL]<<48) |
		(table[(val>>16)&0xFFULL]<<40) | (table[(val>>24)&0xFFULL<<32]) |
		(table[(val>>32)&0xFFULL]<<24) | (table[(val>>40)&0xFFULL<<16]) |
		(table[(val>>48)&0xFFULL]<<8) | (table[(val>>56)&0xFFULL]);
}
